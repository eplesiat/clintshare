...
def subfreva(conf_dict, ans_dict, files, userid, members):
...
"""
    ...
    print(header, file=f)
    print("python -m clintshare.frevadd {} -p {} -i {} -o {} -e {} -m {} -n {} -c {} -r {} -d {} -u {}".format(",".join(files),
                ans_dict["Product"], ans_dict["Institute"], ans_dict["Model"], ans_dict["Experiment"], ",".join(members),
